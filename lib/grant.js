...
function usePasswordGrant (next) {
  ...
  var self = this;var txtmessage = 'User credentials are invalid';
  return this.model.getUser(uname, pword, function (err, user, message) {
    if (err) return next(new error.InternalError(err));
    if (!user) {
      var showMessage = (message == undefined) ? txtmessage : message;
      return next(new error.InvalidCredentialsError(showMessage));
    }
...
function saveAccessToken (next) {
  ...
  var expires = null;
  if (this.config.expiresDate !== null) {
    expires = this.config.expiresDate
  }else{
...
function sendResponse (next) {
  ...
  if (this.config.expiresDate !== null) {
    response.expires_in = new Date(this.config.expiresDate).getTime() / 1000;
  }else{
    if (this.config.accessTokenLifetime !== null) {
